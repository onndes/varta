...
> підрозділі. Збудовано на React 19 + TypeScript + Vite. Дані зберігаються локально в IndexedDB
> (Dexie.js). Без сервера.
...
> _Стсеріншот placeholder — додайте знімок або GIF головного екрану тут._
...
12. [Багатофазна оптимізація свопів](#12-багатофазна-оптимізація-свопів)
13. [Відпустки, відрядження та статуси](#13-відпустки-відрядження-та-статуси)
14. [Load Rate та Anti-Catch-Up](#14-load-rate-та-anti-catch-up)
15. [Ваги днів тижня](#15-ваги-днів-тижня)
16. [Налаштування логіки (AutoScheduleOptions)](#16-налаштування-логіки-autoscheduleoptions)
17. [Каскадний перерахунок](#17-каскадний-перерахунок)
18. [DecisionLog — кнопка «i»](#18-decisionlog--кнопка-i)
19. [Мультибаза (підрозділи)](#19-мультибаза-підрозділи)
20. [Експорт / Імпорт](#20-експорт--імпорт)
21. [Запуск і збірка](#21-запуск-і-збірка)
...
│   │   ├── helpers.ts         — Load Rate, дні активності, SSE, цільова функція Z
│   │   ├── swapOptimizer.ts   — багатофазна оптимізація свопів
│   │   ├── decisionLog.ts     — текст пояснення для кнопки «s»
...
Цей проєкт розроблявся з використанням AI-assisted development tools, зокрема Claude, GPT-4 та
Gemini.
...
| `Заповнити прогалини`  | Автозаповнення порожніх днів (спершу — зведення перед генерацією) |
| `Виправити конфлікти`  | Виправити дні з недоступними бійцями або залишити їх як виняток |
| `Генерувати тиждень`   | Повна регенерація поточного тижня (зі зведенням перед генерацією) |
...
- **AssignmentModal** — модальне вікно призначення: `Замінити / Обмін / Зняти`, список вільних
  бійців з метриками (навантаження, дні очікування), фільтр пошуку.
...
Таблиця всіх бійців: кількість чергувань, зважене навантаження, останнє чергування, статус
доступності.
...
Вкладка інструментів розробника (завжди видима). Містить: генератори даних, тестові сценарії
(стандартна група, відпустки, заблоковані дні, несумісні пари, мала група), генератор
розкладу зі статистикою fairness, швидкий редактор статусу, хаос-режим та повне очищення БД.
...
```
Draft (Жадний прохід)  →  Swap Optimization (4 фази)
```
...
| Weekly Cap         | `filterByWeeklyCap`            | Для ≥7 бійців: макс. 1 наряд на тиждень                      |
...
Компаратор (`buildUserComparator`) порівнює двох кандидатів за 12 ознаками у порядку пріоритету:

| #   | Критерій               | Тип                       | Опис                                                                                   |
| --- | ---------------------- | ------------------------- | -------------------------------------------------------------------------------------- |
| −2  | Cross-DOW Zero Guard   | Жорсткий штраф            | Якщо maxDow > minDow і поточний DOW — максимальний → астрономічний штраф               |
| −1  | forceUseAllWhenFew     | Жорстке переключення      | При ≤7 бійців: 0 нарядів цього тижня → абсолютний пріоритет                            |
...
| 2   | Same-DOW Penalty       | М'який (експоненціальний) | 7д=100, 14д=25, 21д=6.25, >28д=0                                                       |
| 3   | Weekly Cap Tie-break   | Tie-break                 | Менше нарядів цього тижня → вищий (при увімкненому `limitOneDutyPerWeekWhenSevenPlus`) |
| 4   | DOW Recency            | Tie-break                 | Більше днів з останнього призначення на цей DOW → вищий                                |
| 5   | Remaining Availability | forceUse mode             | Менше доступних днів → вищий (терміновість)                                            |
| 6   | Load Rate              | Anti-catch-up             | `Rate = assignments / daysActive` — нижчий rate вперед                                 |
| 7   | Duty Load              | Зважений баланс           | `calculateUserLoad` (при увімкненому `considerLoad`)                                   |
| 8   | Wait Days              | Tie-break                 | Більше днів з останнього наряду → вищий                                                |
| 9   | Stable ID              | Детермінізм               | `a.id − b.id` для відтворюваності                                                      |
...
| `W_TOTAL_SSE`   | 100.0    | Анти-концентрація: запобігає зосередженню нарядів на одному бійцеві |
| `W_WITHIN_USER` | 300.0    | Внутрішньо-користувацький DOW-баланс (патерн 1-1-1-1-1-1-1)         |
| `W_SYSTEM_SSE`  | 3.0      | Базова між-користувацька DOW-справедливість                         |
| `W_LOAD_RANGE`  | 1.0      | М'який тиск на рівномірність навантаження                           |
| `W_ZERO_GUARD`  | 10.0     | Множник (всередині 5000+), ефективно мін. 50 000 за порушення       |
...
## 12. Багатофазна оптимізація свопів
...
└────────────────────────┬────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────┐
│  Фаза 3: Same-DOW Resolution (повтори DOW)          │
...
└────────────────────────┬────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────┐
│  Фаза 4: 3-Way Cyclic Swaps (Ротація трійок)       │
│  Для трійок (d₁,d₂,d₃): ротація A→B→C→A та        │
│  A→C→B→A. Ламає плато, недосяжні 2-way свопами     │
└────────────────────────┬────────────────────────────┘
                         │ покращення? → повернутись до Фази 1
                         ▼
                    Стабільне Z
...
## 16. Налаштування логіки (AutoScheduleOptions)
...
| `dutyPattern`                          | Необов’язковий режим: класичні поодинокі наряди або блочна ротація (`N` діб наряду + `M` відсипного) |
...
| `forceOverrideAccounting`              | Облік force-нарядів у заблоковані/недоступні дні: `'normal'` (як звичайний наряд), `'neutral'` (не впливає на справедливість) |
| `confirmBeforeGeneration`              | Показувати перед «Генерація» / «Заповнити» зведення по періоду (статуси, виключені з авторозподілу, заблоковані дні тижня, нестача людей на тиждень) з кнопками «Продовжити генерацію» / «Скасувати» (за замовчуванням увімкнено) |

---

## 17. Каскадний перерахунок
...
useScheduleActions.runFillGaps()
    ↓
confirmGeneration() → модал перевірки перед генерацією: статуси,
    виключені з авторозподілу, заблоковані дні тижня, нестача людей.
    «Скасувати» перериває; вимикається опцією confirmBeforeGeneration.
    ↓
